...
Every CRITICAL/HIGH insight must trace back to one of the five known Khordha fraud patterns. If a risk doesn't map to a pattern, add a new pattern to docs/fraud-patterns.md and define the detection rule before shipping. The five confirmed patterns are:

1. Patia Industrial-Lease Scam (Sep 2024): Lease-to-freehold land resumption under OGLS Act S.3B. Detection: IGR shows prior lease deed (RLD prefix) on a Sthitiban/Raiyati khata. Fire CRITICAL.
...
4. Industrial-Zone Plot Sold as Residential (CEE DEE Builders pattern): Plot in BDA Industrial Zone sold as residential. Detection: plot GPS cross-referenced against BDA Master Plan zoning → CRITICAL.
5. Subdivided Plot Without BDA Layout Approval: Sub-plot indicator (415/1, D/88) + no BDA layout approval → HIGH WATCH-OUT.
...
Every report renders in three independently shareable layers:
- Layer 1 (WhatsApp Forward): Plot address + owner name + area + top 3 green/amber/red signals + total quantified exposure + timestamp + verification link. Small "Generated by ClearDeed" footer.
- Layer 2 (Buyer's Read): The six Buyer Questions answered with collapsible details. 5-10 screens. This is the primary purchase-decision surface.
- Layer 3 (Lawyer's Drill-Down): Full source-by-source detail. Raw data + parser confidence + provenance links. Viewer is the buyer's engaged advocate.
...
[T-047] [TODO] [P0] Implement Cost-of-Risk Calculator
  Created: 2026-05-14
  Notes: Purely additive to existing insight engine. Every CRITICAL/HIGH insight gets a ₹ financial exposure estimate. Display formats per Section 2 of CLEARDEED_FINAL_CEILING.md. No new fetchers needed. Implement in A10 report writer as `FinancialExposureSummary` panel.

[T-048] [TODO] [P0] Implement Six Buyer Questions summary panel
  Created: 2026-05-14
  Notes: Top-of-fold summary mapping every fetched insight into one of six buckets: Q1 ownership, Q2 buildability, Q3 encumbrance/loss risk, Q4 pricing, Q5 area trajectory, Q6 post-purchase costs. Financial Exposure Summary panel integrated here.

[T-049] [TODO] [P0] Build EOW Khordha blacklist fetcher
  Created: 2026-05-14
  Notes: Monthly scrape of eowodisha.gov.in press releases. Maintain `khordha_eow_blacklist.json` in fetcher data. Detection: plot/khata cross-reference against blacklist → CRITICAL; owner name against EOW arrest list → HIGH WATCH-OUT. Covers Pattern 2 (Surya Nirman fraud).

[T-050] [TODO] [P1] Implement IGR lease-deed detection rule
  Created: 2026-05-14
  Notes: IGR shows any prior lease deed (RLD prefix instead of RSD) on a Sthitiban/Raiyati khata → fire CRITICAL. Covers Pattern 1 (Patia Industrial-Lease Scam). Requires IGR concierge or portal access.

[T-051] [TODO] [P0] Implement seller-identity distance check
  Created: 2026-05-14
  Notes: If seller's claimed address is materially distant from plot district AND no registered PoA at local SRO → HIGH WATCH-OUT. Covers Pattern 3 (Malipada Impersonation Scam). Triggers video KYC + direct owner contact recommendation.

[T-052] [TODO] [P1] BDA layout approval checker
  Created: 2026-05-14
  Notes: Query BDA BPAS-Online / OBPS portals (bda.gov.in) for layout/building permission status. Plot number contains sub-plot indicator (415/1, D/88) + no BDA layout approval entry → HIGH WATCH-OUT. Covers Pattern 5 (Subdivided Plot Without BDA Layout Approval). T-052.
...
[T-064] [TODO] [P2] WhatsApp Forward layer generator
  Created: 2026-05-14
  Notes: Generate Layer 1 WhatsApp Forward: plot address + owner + area + top 3 green/amber/red signals + total quantified exposure + timestamp + verification link + "Generated by ClearDeed" footer. This is the viral surface — every report shared is a marketing touchpoint.
...
Weeks 1-2: Implement Cost-of-Risk Calculator (T-047) + Six Buyer Questions panel (T-048) + WhatsApp Forward generator (T-064). Purely additive, no new fetchers.
Weeks 3-4: T-031 (RCCMS live fetcher) + T-049 (CERSAI public search) + time-series trajectory rules (T-059).
...
Decision: Every CRITICAL/HIGH insight in the report must trace back to one of these five patterns or add a new one to docs/fraud-patterns.md. The five confirmed patterns are:
  ...
  5. Subdivided Plot Without BDA Layout Approval: Sub-divided plots (D/88 pattern) sold without BDA layout approval. Detection: sub-plot indicator present + no BDA layout approval entry → HIGH WATCH-OUT.
Consequences: Every new insight rule must map to a pattern. Patterns live in docs/fraud-patterns.md and are maintained monthly.
...
Decision: Every report renders in three independently shareable layers: Layer 1 (WhatsApp Forward): plot address + owner name + area + top 3 green/amber/red signals + total quantified exposure + timestamp + verification link + "Generated by ClearDeed" footer. Layer 2 (Buyer's Read): six buyer questions answered with collapsible details. 5-10 screens. Layer 3 (Lawyer's Drill-Down): full source-by-source detail, raw data, parser confidence, provenance links.
Consequences: A10 generates all three layers from the same data. Layer 1 is the viral surface — every share is a marketing touchpoint.
...
Decided:
  ...
  - ADR-024: Three-layer report architecture (WhatsApp Forward → Buyer's Read → Lawyer's Drill-Down)
  - ADR-025: Verification feedback loop is the primary moat (30/90/180/365-day buyer follow-up + B2B lawyer layer)
...
In progress:
  ...
  - T-064: WhatsApp Forward layer generator — viral surface
  - T-061: Provenance "Verify yourself" links — elevate DPR-INF-001 to first-class
  - V1.1 Day 9: Concierge queue + WhatsApp delivery workflow
