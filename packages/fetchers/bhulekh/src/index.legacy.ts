...
const PARSER_VERSION = "bhulekh-ror-html-v2";
...
export async function fetch(input: {
  ...
  const resolvedVillageInfo = villageInfo;

  const districtCode = DISTRICT_CODE;
  const tahasilCode = resolvedVillageInfo.bhulekhTahasilCode;
  const villageCode = resolvedVillageInfo.bhulekhVillageCode;
  ...
  const resolvedVillageCode = villageCode;

  if (!input.plotNo && !input.khatiyanNo) {
    return buildFailedResult(
      fetchedAt,
      "Bhulekh v1 requires plotNo or khatiyanNo. Tenant-name mode not yet implemented.",
      resolvedVillageInfo.english,
      { attempts: 0, inputsTried }
    );
  }
  ...
  async function runBhulekhAttempt(attempt: number): Promise<RoRResultType> {
    ...
    // Step 1: Browser bootstrap — establishes ASP.NET session cookies
    console.log("[bhulekh] Browser bootstrap...");
    const browser = await getBrowser();
    ...
    console.log("[bhulekh] Select village:", villageCode);
    ...
    if (input.plotNo) {
      const match = plotOpts.find(
        (o) => o.text.trim() === input.plotNo?.trim() || o.value.trim() === input.plotNo?.trim()
      );
      if (!match) {
        throw Object.assign(
          new Error(`Plot "${input.plotNo}" not found in Bhulekh plot dropdown.`),
...
export function parseRoRHtml(
  ...
  if (tenants.length === 0 && plotRows.length > 0) {
    ...
    const decimil = parseAreaComponent(row.areaDecimals ?? "");
    const totalAcre = acres + decimil / 100;

    tenants.push({
      surveyNo: row.plotNo,
      area: Math.round(totalAcre * 100) / 100,
      unit: "acre" as const,
      ...buildAreaSourceMetadata(row),
      landClass: row.landTypeOdia ?? perPlotLandClass,
